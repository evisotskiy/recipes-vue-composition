<template>
  <form class="form" @submit.prevent="handleSubmit">
    ...
    <div class="buttons">
      ...
      <button class="btn secondary" type="button" @click="toggleVisible">{{ isVisible ? 'Убрать форму' : 'Показать форму' }}</button>
    </div>
  </form>
</template>
...
export default {
  ...
  setup(props) {
    const { visible: isVisible, toggle: toggleVisible } = useToggle(true);

    return { ...useForm(props), isVisible, toggleVisible }
  },
}
</script>
